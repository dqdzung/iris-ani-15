...
const slotTop = (i: number) => TOP_BAR + MARGIN + Math.floor(i / N) * (TILE + GAP);
...
export class SlidingPuzzle extends Phaser.Scene {
  ...
  preload() {
    this.load.image("iris", "/iris-15.jpg");
  }
  ...
  create() {
    ...
    this.add.image(tx, 66 * S, "pic").setDisplaySize(44 * S, 44 * S).setOrigin(0.5);
    this.add
  ...
  private buildPicture() {
    const src = this.add.image(0, 0, "iris").setOrigin(0, 0).setDisplaySize(PB, PB);
  ...
  private placeTile(v: number, slot: number, animate: boolean) {
    ...
    const lx = slotLeft(slot) + 7 * S,
      ly = slotTop(slot) + 5 * S;
    if (animate) {
      this.tweens.add({ targets: t.img, x: ix, y: iy, duration: SLIDE_MS, ease: "Quad.easeOut" });
      this.tweens.add({ targets: t.label, x: lx, y: ly, duration: SLIDE_MS, ease: "Quad.easeOut" });
    } else {
  ...
  private isSolved() {
    for (let i = 0; i < N * N - 1; i++) if (this.grid[i] !== i + 1) return false;
    return true;
  }

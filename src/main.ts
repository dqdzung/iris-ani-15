...
if (import.meta.env.DEV) {
  ...
  // Dev-only skip shortcuts (stripped from prod builds):
  //   K — clear the current game → its IRIS transition
  //   F — jump straight to the finale
  ...
  window.addEventListener("keydown", (e) => {
    if (e.key === "k") {
      ...
      game.scene
        ...
        .forEach(
          (s) =>
            !["Boot", "IrisProgress"].includes(s.scene.key) && s.scene.stop(),
        );
